...
test("hosted collection never scrapes a Tavily-discovered URL", async () => {
  ...
  const evidence = await collectHostedEvidence({
    ...
    tavily: { search: async () => ({ provider: "tavily", state: "success", observedAt: "2026-08-13T00:00:00Z", values: { url: "https://untrusted.example" } }) }
  });
