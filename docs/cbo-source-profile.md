...
2. Review and apply [cbo_public_directory_v1.sql](../sql/source/cbo_public_directory_v1.sql) in the mirror. This creates only a view; it does not change source rows.
3. Provision a separate `cbo_import_reader` role with access only to the view. Do not reuse the profiling credential for runtime import.
