...
export function CalibrationSummary({ summaries }: { summaries: Calibration[] }) {
  return <section className="queue-panel" aria-labelledby="calibration-title">
    ...
    <p>Human decisions are the reference. These aggregate counts do not change verification policy.</p>
    {summaries.length ? <ul className="evidence-list">{summaries.map((summary) => <li key={summary.promptVersion}><strong>{summary.promptVersion}</strong>: {summary.reviewed} reviewed · {summary.aligned}/{summary.comparable} aligned · {summary.disagreed} disagreed · {summary.insufficientEvidence} insufficient evidence · {summary.deferred} deferred</li>)}</ul> : <p className="empty-queue">No reviewed advisory records yet.</p>}
  </section>;
}
