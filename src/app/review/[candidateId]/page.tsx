...
export default function CandidateReviewPage({ params }: { params: { candidateId: string } }) {
  ...
  return `<main><h1>Candidate ${candidate.id}</h1><p>Status: ${candidate.status}</p><pre>${JSON.stringify(candidate.proposedValues, null, 2)}</pre><h2>Evidence</h2><ul>${candidate.evidence.map((item) => `<li>${item}</li>`).join("")}</ul></main>`;
}
